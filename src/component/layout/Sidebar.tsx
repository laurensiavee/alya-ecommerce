...
import { selectIsSidebarOpen, setSidebarOpen } from "@/store/baseSlice";
...
export default function Sidebar() {
    ...
    return (
      <>
        <aside id="default-sidebar" className={isSidebarOpen? "fixed left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" : "fixed left-0 z-40 w-12 h-screen transition-transform -translate-x-full sm:translate-x-0"} aria-label="Sidebar">
            <div className="h-full px-3 py-5 overflow-y-auto bg-l-text">
                <div className="text-d-text text-end">
                    {isSidebarOpen &&
                        <FontAwesomeIcon onClick={handleSidebar} icon={faArrowLeft} className="p-2 hover:text-l-primary hover:bg-l-primary/20 hover:shadow-2xl hover:shadow-l-primary/70 rounded-lg" />
                    }
                    {!isSidebarOpen &&
                        <FontAwesomeIcon onClick={handleSidebar} icon={faArrowRight} className="p-2 hover:text-l-primary hover:bg-l-primary/20 hover:shadow-2xl hover:shadow-l-primary/70 rounded-lg" />
                    }
                </div>
                {isSidebarOpen &&
                    <>
